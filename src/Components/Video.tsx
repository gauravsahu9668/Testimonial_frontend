...
const Video: React.FC<VideoReviewProps> = ({ videoReview }) => {
  ...
  const downloadHandler = (videourl: string) => {
    handleDownload(videourl, "video.mp4")
    toast.success("video start downloading")
  }
  const crossHandler = () => {
    setcode(false);
    setreviewid(-1);
  ...
  const generateifram = async (id: number) => {
    setcode(true)
    setreviewid(id);
    const iframe = `<iframe src="https://backend.sahug6194.workers.dev/api/v1/space/iframevideo/${id}" width="700" height="400" style="border: none;"></iframe>`
    ...
    <>
      {
        videoReview.map((data, index) => {
          return (
            ...
            //                                             <div onClick={()=>{generateifram(data.videoReview_id)}} className="flex cursor-pointer items-center gap-x-1 px-2 rounded-lg hover:bg-[#c5bebe]">
            //                                                <IoMdCodeWorking></IoMdCodeWorking>
            //                                                 code
            //                                             </div>
            //                                             <div onClick={()=>{downloadHandler(data.videoUrl)}}  className="flex cursor-pointer items-center px-2 gap-x-1 rounded-lg hover:bg-[#c5bebe]">
            ...
            // </div>
            // {
            //     code && data.videoReview_id===reviewid && <div className="p-6 relative rounded-lg mx-auto bg-[#1F2937] text-white sm:w-[70%] w-full mb-10">
            ...
            <div key={index} className="flex rounded-lg  shadow-[0_0_50px_rgba(16,185,129,0.15)] hover:shadow-[0_0_50px_rgba(16,185,129,0.25)] transition-shadow duration-300 bg-[#111113] flex-col m-4 p-2 w-full ">
              <div className="w-full  flex items-center justify-between">
                ...
                <div onClick={() => { likeHandler(data.videoReview_id, !data.Liked, "text") }} className=" w-[40px] h-[40px] group ">
                  {
                    data.Liked === true ? <FcLike size={"2rem"} className="cursor-pointer" ></FcLike> : <FcLikePlaceholder size={"2rem"} className="cursor-pointer"></FcLikePlaceholder>
                  }
                </div>
              ...
              <div className="w-full flex justify-between  items-center p-2">
                <div className=" px-3 py-2 text-[#706f6f] font-semibold flex items-center justify-between">
                  <div onClick={() => { generateifram(data.videoReview_id) }} className="px-3 py-2 rounded-md text-sm font-medium text-gray-300 hover:text-white hover:bg-black/40 
...
hover:shadow-lg hover:shadow-emerald-500/10  gap-x-1  hover:bg-[#c5bebe]">
                  ...
                  <div onClick={() => { downloadHandler(data.videoUrl) }} className="px-3 py-2 rounded-md text-sm font-medium text-gray-300 hover:text-white hover:bg-black/40 
flex items-center space-x-2 transition-all duration-200 ease-in-out cursor-pointer
hover:shadow-lg hover:shadow-emerald-500/10  gap-x-1  hover:bg-[#c5bebe]">
                    <MdDownload></MdDownload>
                    download
...
hover:shadow-lg hover:shadow-emerald-500/10  gap-x-1  hover:bg-[#c5bebe]">
              ...
              {
                code && data.videoReview_id === reviewid &&
                <div className="p-6 relative rounded-lg mx-auto bg-[#18181B] text-white w-[300px] sm:w-[70%]  mb-10 ">
                  <div onClick={() => { crossHandler() }} className="w-[25px] absolute h-[25px] right-2 top-2 flex items-center justify-center text-black bg-white rounded-md cursor-pointer"><MdDeleteOutline></MdDeleteOutline></div>
