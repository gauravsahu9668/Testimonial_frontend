...
const Classic: React.FC<PostCardProps> = ({ item }) => {
...
>
      <div className="flex items-center gap-3 mb-2">
        <img
          ...
          className={`w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 border-[1px] rounded-full object-cover ${picture ? "block" : "hidden"}`}
        />
        <div className="flex flex-col">
          <div className="flex items-center gap-1">
            <p style={{ color: nameColor }} className={`font-semibold text-base sm:text-lg md:text-xl ${name ? "block":"hidden"}`}>
              {item.custName}
            </p>
          ...
          <p style={{ color: captionColor }} className={`text-xs sm:text-sm md:text-base ${email ?"block":"hidden"}`}>
            {item.custEmail}
          </p>
          <p style={{ color: captionColor }} className={`text-xs sm:text-sm text-gray-500 ${date ? "block":"hidden"}`}>
            {new Date(item.createdAt).toLocaleDateString()}
          </p>
        </div>
      </div>
      <div className={`flex mt-1 sm:mt-2 ${rating ? "block" : "hidden"}`}>
        {Array.from({ length: item.starRating }).map((_, idx) => (
          <Star key={idx} className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6" style={{ color: ratingColor, fill: ratingColor }} />
        ))}
      </div>
      <div style={{ color: reviewTextColor }} className="text-sm sm:text-base md:text-lg leading-relaxed mt-1 mb-2">
        {expanded
          ? item.custMessage
      ...
      {item.custMessage.length > 100 && (
        <button
          ...
          className="font-medium text-xs sm:text-sm md:text-base self-start"
        >
